...
- `keramos version` prints just the version when the commit and build date
  are not embedded, which is the case for any binary installed with
  `go install`, instead of padding the line with `unknown`.

### Removed

- The NetBSD arm64 release target. The C runtime keramos's storage layer
  depends on has no arm64 support on NetBSD, so the target could never
  produce a binary.
...
- The import path carries the major version, as Go requires from v2 onward:
