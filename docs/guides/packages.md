...
│   ├── staging.yaml
│   └── prod.yaml
├── policies/                       # optional — package-defined policy rules (.rego or keramos policy YAML)
...
Optional. Package-defined policies that `keramos policy run` evaluates against the rendered manifest. Two formats accepted:
